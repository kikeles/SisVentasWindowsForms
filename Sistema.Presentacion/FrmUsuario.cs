...
namespace Sistema.Presentacion
{
    public partial class FrmUsuario : Form
    {
        ...
        private void Formato()
        {
            ...
            DgvListado.Columns[2].Visible = false;//IdRol oculta
        ...
        private void BtnInsertar_Click(object sender, EventArgs e)
        {
            ...
            {
                string Rpta = "";
                if (CboRol.Text == string.Empty || TxtNombre.Text == string.Empty
                ...
                else
                {
                    ...
                    Rpta = NUsuario.Insertar(Convert.ToInt32(CboRol.SelectedValue),TxtNombre.Text.Trim(), 
                        CboTipoDocumento.Text,TxtNumDocumento.Text.Trim(),TxtDireccion.Text.Trim(),
                        TxtTelefono.Text.Trim(),TxtEmail.Text.Trim(),TxtClave.Text.Trim());
                    if (Rpta.Equals("OK"))
                    {
        ...
        private void BtnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //Rpta (Respuesta) proviede de la capa Datos pasando por la capa Negocio
                    Rpta = NUsuario.Actualizar(Convert.ToInt32(TxtId.Text),Convert.ToInt32(CboRol.SelectedValue), TxtNombre.Text.Trim(),
                        CboTipoDocumento.Text, TxtNumDocumento.Text.Trim(), TxtDireccion.Text.Trim(),
        ...
        {
            this.Limpiar();
            TabGeneral.SelectedIndex = 0;//regresa a la tabla listado
        }
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DgvListado.Rows)
                    {
                        ...
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            if (Rpta.Equals("OK"))
                            {
                                this.MensajeOk("Se desactivó el registro" + row.Cells[4].Value.ToString());
                            }
                            else
                            {
                                this.MensajeError(Rpta);
